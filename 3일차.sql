-- 비동등결합(NON-EQUI JOIN) : 두개 이상의 테이블에 결합조건에 = 연산자가 아닌 다른 연산자를 사용하여
-- 참(TRUE)인 행을 결합하여 검색
...
-- 포함되는 행을 결합하여 검색
SELECT EMP, ENAME, SAL, GRADE FROM EMP, SALGRADE WHERE SAL BETWEEN LOSAL AND HISAL;

-- 외부결합(OUTER JOIN): 결합조건이 참인 행만 결합하여 검색하는 것이 아니라 결합조건이 맞지 않는 행도
-- NULL과 결합하여 검색
-- 결합 조건식의 테이블에 (+)를 사용하면 결합조건이 맞지 않는 행을 NULL과 결합하여 검색

EMP 테이블에 저장된 모든 사원의 부서번호를 중보되지 않는 유일한 컬럼값 검색
SELECT DISTINCT DEPTNO FROM EMP; -- 검색결과 : 10, 20, 30

-- DEPT 테이블에 저장된 모든 부서정보의 부서번호, 부서이름, 부서위치 검색
SELECT DEPTNO, DNAME, LOC FROM DEPT; -- 검색결과 : 10, 20, 30,40

-- EMP 테이블과 DEPT 테이블에 저장된 모든 사원의 사원번호, 사원이름, 급여, 부서이름, 부서위치 검색
-- 결합조건 : EMP 테이블의 부서번호(DEPTNO)와 DEPT 테이블의 부서번호(DEPT)가 같은 행만 결합
-- 40번 부서에 근무하는 사원이 없으므로 40번 부서에 대한 부서이름, 부서위치 검색되지 않음
...
-- 자기결합(SELF JOIN) : 하나의 테이블을 서로 다른 별칭을 부여하고 2개 이상의 테이블로 구분하여 행을 결합
...
-- 결합조건 : EMP 테이블(WORKER)의 관리자 번호(MGR)와 EMP 테이블(MANAGER)의 사원번호(EMPNO)가 같은 행을 결합
...
-- JOIN USING : 결합 테이블에 같은 이름의 컬럼이 여러개 있는 경우 같은 이름의 컬럼에 저장된 컬람값으로
-- 행을 결합하여 검색 - 결합조건 생략
