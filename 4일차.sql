...
-- UNION : 두 개의 SELECT 명령으로 검색된 행을 합하여 결과값을 제공하는 연산자 - 중복행 제외
...
SELECT * FROM SUPER_HERO UNION SELECT * FROM MARVEL_HERO; -- 검색대상의 갯수가 다르므로 에러 발생

-- UNION ALL : 두 개의 SELECT 명령으로 검색된 행을 모두 합친 행을제공하는 연산자 - 중복행 포함
...
-- INTERSECT : 두개의 SELECT 명령으로 검색된 행에서 중복된 행을 제공하는 연산자
...
SELECT NAME FROM SUPER_HERO UNION SELECT NAME, GRADE FROM MARVEL_HERO; -- 검색 대상의 갯수가 다르므로 에러 발생
...
-- 테이블에 삽입될 행의 컬럼값은 테이블 속성의 순서대로 자료형에 맞는 컬럼값을 생략없이 차례대로 나열해서
-- 전달해야만 행 삽입 처리
...
INSERT INTO DEPT VALUES (60, '총무부', '수원', '팔달구'); -- 전달값이 너무 많아 에러 발생

-- 삽입행으로 전달될 컬럼값의 자료형이 테이블의 컬럼의 자료형과 맞지 않을 경우 에러 발생
INSERT INTO DEPT VALUES('육십', '총무부', '팔달구'); -- 숫자값이 아닌 컬럼값을 사용하여 에러 발생

-- 삽입행으로 전달될 컬럼값이 테이블 컬럼의 크기보다 큰 경우 에러 발생
INSERT INTO DEPT VALUES(6000, '총무부', '수원'); -- 부서번호에 2자의 숫자값보다 큰 값을 사용하여 에러 발생
...
-- DEPT 테이블의 DEPTNO 컬럼에는 PK 제약조건이 부여

SELECT DISTINCT DEPTNO FROM DEPT; -- 검색결과 : 10, 20, 30, 40, 50
INSERT INTO DEPT VALUES(50, '총무부', '수원'); -- PK 제약조건이 부여된 컬럼에 중복값을 전달하여 에러 발생
-- 테이블 속성과 제약조건을 위반하지 않는 컬럼값을 전달해야만 행 삽입 가능
...
-- DEPT 테이블 구조 확인
DESC DEPT; -- 컬럼의 NULL 허용 유무 확인
-- NULL을 허용하지 않는 컬럼(NOT NULL)에는 NULL 저장 불가능
INSERT INTO DEPT VALUES(NULL, '영업부', '인천시'); -- NOT NULL 컬럼에 NULL 전달하여 에러 발생
...
-- 테이블 생성 또는 테이블의 컬럼 변경시 컬럼에 저장될 기본값 설정 및 변경 가능
...
-- 날짜형 컬럼에는 날짜값 대신 SYSDTE 키워드를 사용하여 컬럼값 전달 가능
...
-- 일반적으로 PK 제약조건이 부여된 컬럼값은 변경하는 것을 권장하지 않는다
...
-- DEPT 테이블에서 부서이름이 영업부인 부서위치(NULL)를 부서이름이 총무부인 부서위치(수원시)로 변경
SELECT * FROM DEPT;
...
-- BONUS 테이블에서 사원이름이 KIM인 사원보다 성과급이 적은 사원의 성과급이 100 증가되도록 변경
 ...
 -- FK(FOREIGN KEY) 제약조건 : 자식 테이블의 컬럼값이 부모 테이블의 컬럼값을 참조하여 저장하는 기능을 제공하는 제약조건
 ...
 DELETE FROM DEPT WHERE DEPTNO = 20; -- 자식 테이블이 참조하는 부모 테이블의 행을 삭제할 경우 에러 발생
 ...
 DESC DEPT;
 CREATE TABLE MERGE_DEPT(DEPTNO NUMBER(2), DNAME VARCHAR(14), LOC VARCHAR(13));
 DESC MERGE_DEPT;
...
-- 트렌젝션에 저장된 SQL 명령을 실제 테이블에 적용하기 위해서는 커밋 처리 - 커밋 처리후 트렌젝션 초기화
...
-- 트렌젝션에 저장된 SQL 명령을 실제 테이블에 적용하지 않고 초기화하기 위해서는 롤백(ROLLBACK) 처리
...
-- DEPT 테이블의 검색결과에서 트렌젝션에 저장된 DELETE 명령을 실행한 결과를 제공
SELECT * FROM DEPT;
