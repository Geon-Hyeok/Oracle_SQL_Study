...
-- EMP 테이블에서 사원번호가 7788인 사원정보를 검색하여 사원번호, 사원이름, 업무, 급여, 업무별 급여 실지급액을
-- 검색하여 출력하는 PL/SQL 작성
...
-- EMP 테이블에서 사원번호가 7788인 사원의 사원정보를 검색하여 사원번호, 사원이름, 급여 급여등급을 계산하여 출력하는 PL/SQL 작성
...
--          FETCH 커서명 INTO 변수명, 변수명, ...; /* 커서 위치의 검색행을 제공받아 변수에 저장 - 커서는 다음 행으로 이동 */
...
-- 저장 프로시저 호출 - 저장 프로시저에 작서된 PL/SQL 실행
...
-- 저장 프로시저 생성시 발생된 컴파일 에러가 발생된 경우 컴파일 로그를 이용하여 확인 가능
SHOW ERROR;
...
-- 2. OUT : 저장 프로시저 호출시 바인딩 변수를 전달받아 저장 프로시저의 PL/SQL 실행 결과를 
-- 저장하여 외부에 결과값을 제공할 목적의 매개변수를 선언할 때 사용
...
-- 사원번호를 매개변수로 전달받아 EMP 테이블에서 전달받은 사원번호의 사원정보를 검색하여 급여의 2배에 해당하는
-- 결과값을 반환하는 저장 함수 생성
...
-- EMP_HIS 테이블의 행으로 삽입하는 트리거 생성
CREATE OR REPLACE TRIGGER EMP_HIS_TRIGGER AFTER INSERT OR UPDATE OR DELETE ON EMPP_TRI FOR EACH ROW
BEGIN
    ...
    ELSEIF UPDATING THEN /* UPDATE 명령이 실행된 경우 */
        IF :NEW.DEPTNO <> :OLD.DEPTNO THEN
            INSERT INTO EMP_HIS VALUES(:OLD.EMPNO, :NEW.ENAME, '부서이동');
        ELSEIF
