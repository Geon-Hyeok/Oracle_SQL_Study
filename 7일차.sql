...
-- USER3 테이블의 인덱스 확인 
-- USER_INDEXS : 인덱스 정보를 제공하는 딕셔너리
-- USER_IND_COLMUNS 컬럼에 설정된 인덱스 컬럼에 설정된 인덱스 정보를 제공하는 딕셔너리
...
-- 형식) CREATE INDEX 인덱스면 ON 테이블명(컬럼명)
...
-- SYS.USER_TABLES 객체의 공용 동의어로 USER_TABLES 제공
SELECT TABLE_NAME FROM USER_TABLES;
-- SYS.USER_TABLES 객체의 공용 동의어로 TABS 제공
...
-- 시스템 관리자(SYSDBA - SYS 계정)로 접속하여 현재 접속 사용자(SCOTT 계정)에게 CREATE SYNONYM 시스템 권한을 부여
...
-- 계정 상태 변경 - OPEN(계정 활성화 - DBMS 접속 가능), LOCK(계정 비활성화 - DBMS 서버 접속 불가능)
-- 오라클 서버 접속시 계정의 비밀버호를 5번 틀리면 계정의 상태가 자동으로 LOCK 상태로 변경
...
-- 형식) ALTER USER 계정명 DEFAULT TABLESPACES 테이블스페이스명
...
-- 오라클 권한(ORACLE PRIVILEGE) : 시스템 권한(관리자), 객체 권한(일반 사용자)

-- 시스템 권한 : 시스템을 구성하는 객체를 관리하기 위한 명령(DDL)에 대한 사용 권한
-- 시스템 권한을 계정에게 부여
-- 형식) GRANT {PRIVILEGE | ROLE}, {PRIVILEGE | ROLE}, ... TO {계정명 | PUBLIC} [WITH ADMIN OPTION] [IDENTIFIED BY 비밀]
...
-- CREATE SESSION 권한이 없기에 서버 접속 실패

-- 시스템 관리자(SYSDBA - SYS 계정)가 KIM 계정에게 CREATE SESSION 권한 부여
...
-- 객체 권한은 INSERT, UPDATE, DELETE, SELECT 등으 ㅣ키워드로 표현
...
-- KIM 계정으로 SCOTT 사용자 스키마에 존재하는 DEPT 테이블에 저장된 모든 행 검색 - SQLPLUS
...
-- 오라클에서 기본적으로 제공되는 툴 - CONNECT, RESOURCE, DBA 등
...
-- 오라클 서버에 LEE 계정으로 접속하여 SQWON 테이블 생성 - SQLPLUS 이용 : 서버 접속 및 테이블 생성 가능
...
-- 테이블 변수 : 테이블에 저장된 행을 여러 개 검색하여 얻은 다수의 컬럼값을 저장하기 위해 선언하는 변수 - 배열
-- 형식) 테이블변수명 테이블타입명
...
DECLARE
    ...
    /* 테이블 변수의 요소를 반복처리 하기 위한 첨자 역활의 변수 선언 */
    I BINARY_INTEGER := 0;
BEGIN
...
-- 사원번호가 7844인 사원의 사원번호, 사원이름, 업무, 급여, 부서번호를 검색하여 레코드 변수에 저장 후 출력하느 PL/SQL 작성
DECLARE
    /* 레코드 타입 선언 */
