-- DBMS 서버에 전달된 SQL 명령이 잘못된 경우 롤백 처리를 통한 데이터 복구를 위해 트렌젝션 사용 
-- 데이터 무결성
SELECT * FROM EMP;
...
-- 현재 세션에서 작업중인 테이블의 행을 커밋 처리 전까지 다른 세션에서 검색되지 않도록 트렌젝션 사용 
-- 데이터 일관성
...
-- 현재 세션에서 커밋 처리하기 전까지 다른 세션에서는 기존행의 검색결과 제공 - 데이터 일관성
-- 커밋 처리를 이용해 트렌젝션에 저장된 SQL 명령을 실제 테이블에 적용하면 다른 세션에서도 테이블의 조작된 행 검색
COMMIT;

-- 데이터 잠금기능(LOCK)을 제공하기 위해 트렌젝션 사용
-- DBMS 프로그램은 다중 사용자 환경으로 같은 테이블의 행을 다른 세션에서 조작 가능
-- 현재 세셔에서 작업중인 테이블의 행을 다른 세션에서 작업하지 못하도록 트렌젝션을 사용하여 데이터 잠금 기능 제공

-- BONUS 테이블에서 사원이름이 ALLEN인 사원의 급여를 2000으로 변경
SELECT * FROM BONUS;
UPDATE BONUS SET SAL = 2000 WHERE ENAME = 'ALLEN'; -- 테이블 행에 대한 데이터 잠금 기능 활성화
SELECT * FROM BONUS;

-- 다른 세션을 사용하여 BONUS 테이블에서 사원이름이 ALLEN인 사원의 성과급을 급여의 50%로 변경
-- UPDATE BONUS SET COMM = SAL*0.5 WHERE ENAME = 'ALLEN';
-- 현재 세션에서 작업중인 테이블의 행을 다른 세션에서 조작할 경우 트렌젝션의 데이터 잠금 기능으로 인해
-- 다른 세션이 일시적으로 중지
-- 현재 세션에서 작업중인 테이블의 행에 대한 트렌젝션의 명령을 커밋 또는 롤백 처리해야만 데이터 잠금
-- 기능이 비활성화 되어 다른 세션의 DML 명령이 실행 가능
COMMIT;

-- SAVEPOINT : 트렌젝션에 라벨(위치정보)을 부여하는 명령
-- 트렌젝션에 저정된 DML 명령 중 라벨을 이용하여 원하는 위치의 DML 명령만 롤백 처리하기 위해 사용
-- 형식) SAVEPOINT 라벨명
...
-- SAVEPOINT 명령으로 설정된 라벨을 이용하여 ROLLBACK 처리
-- 형식) ROLLBACK TO 라벨명
...
-- 데이터베이스의 객체(테이브루, 뷰, 시퀀스, 인덱스, 동의어, 사용자 등)를 관리하기 위한 SQL 명령

-- 테이블(TABLE) : 테이터베이스에서 데이터(행)을 저장하기 위한 가장 기본적인 객체
...
-- 2. A~Z, 0~9, _, $, # 문자들을 조합하여 작성 - 대소문자 구분 X, 스네이크 표기법 사용 권장
-- 3. 영문자외에 다른 문자 사용 가능(한글 등) - 권장하지는 않음
-- 4. 키워드로 식별자를 선언할 경우 에러 발생 - " " 안에 표현하면 가능하지만 권장하지는 않음
...
-- USER TABLES 딕셔너리 대신 동의어(SYNONYM)로 TABS 제공
...
-- SALESMAN 테이블에 행 삽입 - 컬럼을 생략하여 삽입 처리한 경우 생략된 컬럼에는 컬럼 기본값이 전달되어 삽입
-- 테이블 생성시 컬럼 기본값을 설정하지 않은 경우 자동으로 NULL이 기본값으로 자동 설정
INSERT INTO SALESMAN(NO, NAME) VALUES(2000, '임꺽정');
SELECT * FROM SALESMAN;
COMMIT;

-- 테이블 생성시 제약조건을 설정하지 않은 경우 컬럼에 어떠한 값을 전달해도 삽입 처리 - 데이터 무결성 위반 가능
INSERT INTO SALESMAN VALUES(1000, '전우치', '10/10/10');
SELECT * FROM SALESMAN;
COMMIT;

-- MANAGER 테이블 생성 - 사원번호(숫자형), 사원이름(문자형), 입사일(날짜형 - 기본값:현재), 급여(숫자형 - 기본값:1000)
CREATE TABLE MANAGER(NO NUMBER(4), NAME VARCHAR2(20), STARTDATE DATE DEFAULT SYSDATE
...
-- CHECK 제약조건을 컬럼수준의 제약조건 설정 - 해당 컬럼을 이용한 조건식 설정 가능(다른 컬럼을 이용한 조건식을 사용하면 에러)
...
-- CHECK 제약조건을 테이블 수준의 제약조건으로 설정
CREATE TABLE SAWON4(NO NUMBER(4), NAME VARCHAR2(20)
    , PAY NUMBER, CONSTRAINT SAWON4_PAY_CHECK CHECK(NAME IS NOT NULL AND PAY>=5000000));
    
-- 제약조건 확인
SELECT CONSTRAINT_NAME, CONSTRAINT_TYPE, SEARCH_CONDITION FROM USER_CONSTRAINTS WHERE TABLE_NAME = 'SAWON4';

-- NOT NULL : NULL을 허용하지 않는 제약 조건 - 컬럼에 반드시 값을 저장되도록 설정하는 제약조건
...
INSERT INTO USER2 VALUES('ABC', '홍길동', '010-1234-5678'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER2 VALUES('ABC', '임꺽정', '010-8765-4321'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER2 VALUES('XYZ', '임꺽정', '010-1234-5678'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER2 VALUES('XYZ', '임꺽정', '010-8765-4321'); -- 아이디와 전화번호가 중복되지 않아 삽입 처리
SELECT * FROM USER2;
COMMIT;
...
CREATE TABLE USER3(ID VARCHAR2(20), NAME VARCHAR(30), PHONE VARCHAR2(15)
...
INSERT INTO USER3 VALUES('ABC', '홍길동', '010-1234-5678'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER3 VALUES('ABC', '임꺽정', '010-8765-4321'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER3 VALUES('XYZ', '임꺽정', '010-1234-5678'); -- UNIQUE 제약조건을 위반하여 에러 발생
SELECT * FROM USER3;
COMMIT;

-- USER4 테이블 생성 - 아이디(문자형), 이름(문자형), 전화번호(문자형) 
-- 아이디와 전화번호를 그룹화하여 UNIQUE 제약조건 설정 - 테이블 수준의 제약조건으로만 설정 가능
...
INSERT INTO USER4 VALUES('ABC', '홍길동', '010-1234-5678'); -- UNIQUE 제약조건을 위반하여 에러 발생
INSERT INTO USER4 VALUES('ABC', '임꺽정', '010-8765-4321'); 
...
INSERT INTO MGR1 VALUES(NULL, '임꺽정', SYSDATE); -- PRIMARY KEY 제약조건을 위반하여 에러 발생 - NULL
...
INSERT INTO MGR2 VALUES(NULL, '임꺽정', SYSDATE); -- PRIMARY KEY 제약조건을 위반하여 에러 발생 - NULL
...
CREATE TABLE MGR3(NO NUMBER(4), NAME VARCHAR(20), STARTDATE DATE
    , CONSTRAINT MGR3_NO_PK PRIMARY KEY(NO, NAME));
DESC MGR3;
