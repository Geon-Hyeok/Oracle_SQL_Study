...
-- 날짜값은 [RR/MM/DD] 패턵을 기본으로 사용하지만 [YYYY-MM-DD] 패턴도 사용 가능
...
-- DECODE(전달값, 비교값1, 변경값1, 비교값2, 변경값2, ...[,기본값) : 전달값을 비교값과 차례대로 비교하ㅇ
...
-- 결합 테이블에 같은 이름의 컬럼을 동시에 검색할 경우 첫번째로 검색되는 컬럼을 제외한 나머지 컬럼의 이름은 자동을 변경되어 검색
...
-- 테이블 결합시 테이블의 이름을 간단하게 표현하기 위해 사용하는 사용하거나 하나의 테이블을 다수의 테이블로 표현하기 위해 사용
