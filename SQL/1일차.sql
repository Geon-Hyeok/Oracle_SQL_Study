...
-- EMP 테이블에서 사원이름이 A로 시작하는 사원의 사원번호,사원이름,업무,급여 검색
...
-- EMP 테이블에서 업무가 MANAGER가 아닌 사원의 사원의 사원번호, 사원이름, 업무, 급여 검색
...
SELECT EMPNO,ENAME,JOB,INSTR(JOB,'A',1,2) FROM EMP WHERE EMPNO=7499; -- (JOB,'A',1,2) 1 = 첫번째 위치부터 찾기 시작 / 2 = 해당하는 문자 중 두번째에 있는 단어
